                    ...
                    Some(Yaml::Hash(info)) => {
                        match info.get(&Yaml::String(String::from("sdkInfo"))) {
                            Some(&Yaml::Hash(ref sdk_info)) => {
                                if let Some(&Yaml::String(ref min_sdk_str)) =
                                    sdk_info.get(&Yaml::String(String::from("minSdkVersion"))) {
...
impl Manifest {
    pub fn load<P: AsRef<Path>, S: AsRef<str>>(path: P,
        ...
        let metadata = PackageMetadata::from_apktool(apktool_path)?;
                        ...
                        "uses-permission" => {
                            for attr in attributes {
                                if let "name" = attr.name.local_name.as_str() {
                                    let permission = if let Ok(p) = Permission::from_str(attr.value
                                        .as_str()) {
                                        p
                                    } else {
                                        let line =
...
mod tests {
    ...
    fn it_do_not_load_on_invalid_apktool() {
        let _ = PackageMetadata::from_apktool("tests/apktool/invalid.yml").unwrap();
    }
}
